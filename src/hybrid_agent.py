...
Based on the formal definition: A_hybrid = ⟨ID, State_reactive+deliberative,
Goals, Decision_layered, Perception, Action⟩
...
class HybridAgent(AbstractAgent):
    ...
    - Decision_layered: reactive layer handles immediate stimuli, deliberative
    layer for planning
    - Combines immediacy of reactive behavior with flexibility of deliberative
    reasoning
    ...
    def __init__(self, agent_id: AgentId, observable_properties: Set[str]):
        ...
        self.intentions: List[Intention] = []  # BDI-style intentions
        self.plan_library: Dict[str, List[str]] = {}  # Plan library
        ...
        self.emergency_threshold: float = (
            8.0  # Threshold for emergency reactive rules
        )
    ...
    def initialize_agent(self):
        """
        Initialize hybrid agent with both reactive rules and deliberative
        capabilities.
        """
        ...
        # Default emergency reactive rule (can be overridden)
    ...
    def decide_action(self) -> str:
        ...
        if self.reactive_layer_enabled:
            emergency_rules = [
                r
                for r in self.decision.reactive_rules
            ...
            for rule in sorted(
                ...
                if (
                    rule.condition(state_dict)
                    and rule.action in available_actions
                ):
        ...
        if self.reactive_layer_enabled:
            ...
            for rule in sorted(
                standard_rules, key=lambda r: r.priority, reverse=True
            ):
                state_dict = {
                    "internal": {
                        k: v.proposition
        ...
        if self.deliberative_layer_enabled:
            ...
            if (
                deliberative_action
                and deliberative_action in available_actions
            ):
    ...
    def perceive(self, environment_state: Dict):
        """
        Hybrid perception: maintain both reactive perceptions and
        deliberative beliefs.
        """
        # Standard perception update
    ...
    def step(self, environment_state: Dict) -> tuple[str, Dict]:
        """
        Hybrid agent step: optimized for both reactive speed and
