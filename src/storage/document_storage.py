...
class MongoDBStorage(DocumentStorage):
    ...
    def __init__(self, config: Dict[str, Any]):
        if not MONGO_AVAILABLE:
            raise ImportError("pymongo package not installed. Install with: pip install pymongo")
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.client = MongoClient(
                connection_string,
                serverSelectionTimeoutMS=self.config.get('server_selection_timeout', 30000),
            ...
            logger.info(f"Connected to MongoDB: {host}:{port}/{self.database_name}")
            return True
    ...
    def insert_one(self, collection: str, document: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(f"Inserted document in collection {collection}: {result.inserted_id}")
    ...
    def insert_many(self, collection: str, documents: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            ...
            logger.debug(f"Inserted {len(documents)} documents in collection {collection}")
    ...
    def find_one(self, collection: str, filter_dict: Dict[str, Any]) -> Optional[Dict]:
    ...
    def find_many(self, collection: str, filter_dict: Dict[str, Any],
                 ...
                 sort: Optional[List[Tuple[str, int]]] = None) -> List[Dict]:
        ...
        try:
            ...
            logger.debug(f"Found {len(results)} documents in collection {collection}")
    ...
    def update_one(self, collection: str, filter_dict: Dict[str, Any],
        ...
        try:
            ...
            if success:
                ...
                logger.warning(f"No document updated in collection {collection}")
    ...
    def update_many(self, collection: str, filter_dict: Dict[str, Any],
        ...
        try:
            ...
            self._record_operation('update_many', start_time, modified_count > 0)
            
            logger.debug(f"Updated {modified_count} documents in collection {collection}")
    ...
    def delete_many(self, collection: str, filter_dict: Dict[str, Any]) -> int:
        ...
        try:
            ...
            self._record_operation('delete_many', start_time, deleted_count > 0)
            
            logger.debug(f"Deleted {deleted_count} documents from collection {collection}")
    ...
    def create_index(self, collection: str, keys: List[Tuple[str, int]]) -> bool:
