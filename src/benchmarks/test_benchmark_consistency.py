...
Verifies that all communication protocols have consistent benchmark implementations
...
try:
    from benchmarks.rest_benchmark_scenarios import create_benchmark_scenarios as create_rest_scenarios
    ...
    from benchmarks.kafka_benchmark_scenarios import create_kafka_benchmark_scenarios
...
class BenchmarkConsistencyTester:
    ...
    def test_scenario_consistency(self) -> Dict[str, List[str]]:
        ...
        for protocol_name, factory_func in self.protocols.items():
            try:
                ...
                missing_scenarios = set(self.expected_scenarios) - set(available_scenarios)
                extra_scenarios = set(available_scenarios) - set(self.expected_scenarios)

                print(f"  {protocol_name:>6}: {len(available_scenarios)} scenarios")
                if missing_scenarios:
                    print(f"    ❌ Missing: {list(missing_scenarios)}")
                if extra_scenarios:
                    print(f"    ℹ️  Extra: {list(extra_scenarios)}")
                if not missing_scenarios and not extra_scenarios:
                    print(f"    ✅ All expected scenarios present")
                    ...
                    try:
                        if scenario_name in benchmark.scenarios:
                            ...
                            if scenario.setup_func:
                                # This should not raise an exception with standard params
                                setup_result = scenario.setup_func(test_params.copy())
                                ...
                                if scenario.teardown_func and setup_result:
                                    try:
                                        ...
                                        pass  # Ignore teardown errors in testing
                            else:
                                protocol_results[scenario_name] = False
                                print(f"    ❌ {scenario_name}: No setup function")
                        else:
                            protocol_results[scenario_name] = False
    ...
    def test_benchmark_framework_integration(self) -> Dict[str, bool]:
        """Test that all protocols integrate properly with the benchmark framework."""
        print("\n🏗️  Testing benchmark framework integration...")
        ...
        for protocol_name, factory_func in self.protocols.items():
            try:
                ...
                required_attributes = ['scenarios', 'run_scenario', 'latency_tracker', 'throughput_tracker']
                has_all_attributes = all(hasattr(benchmark, attr) for attr in required_attributes)

                if has_all_attributes:
                    print(f"  ✅ {protocol_name}: Framework integration OK")
                    results[protocol_name] = True
                else:
                    missing_attrs = [attr for attr in required_attributes if not hasattr(benchmark, attr)]
                    print(f"  ❌ {protocol_name}: Missing attributes: {missing_attrs}")
    ...
    def test_metric_consistency(self) -> Dict[str, Dict[str, bool]]:
        ...
        results = {}
        expected_metrics = [
            'message_latency_avg', 'message_latency_p95', 'message_latency_p99',
            'throughput_avg', 'success_rate', 'cpu_usage_avg', 'memory_usage_avg',
            'total_messages', 'test_duration'
        ]
        ...
        for protocol_name, factory_func in self.protocols.items():
            ...
            try:
                ...
                # We can't run full tests here, so we check if the trackers exist
                if hasattr(benchmark, 'latency_tracker') and hasattr(benchmark, 'throughput_tracker'):
                    # Check if trackers have expected methods
                    latency_methods = ['start_message_timing', 'end_message_timing', 'get_latency_stats']
                    throughput_methods = ['record_message', 'get_current_throughput']

                    has_latency_methods = all(hasattr(benchmark.latency_tracker, method) for method in latency_methods)
                    has_throughput_methods = all(hasattr(benchmark.throughput_tracker, method) for method in throughput_methods)

                    if has_latency_methods and has_throughput_methods:
                        print(f"    ✅ Metric tracking structure OK")
                        protocol_results['metric_structure'] = True
                    else:
                        print(f"    ❌ Missing metric tracking methods")
                        protocol_results['metric_structure'] = False
                else:
                    print(f"    ❌ Missing metric trackers")
                    protocol_results['metric_structure'] = False
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        scenario_success = sum(1 for p, scenarios in all_results['scenario_consistency'].items()
                             if set(scenarios) == set(self.expected_scenarios))
        print(f"Scenario Consistency:    {scenario_success}/{total_protocols} protocols")

        # Parameter consistency
        param_success = sum(1 for p, scenarios in all_results['parameter_consistency'].items()
                          if all(scenarios.values()))
        print(f"Parameter Consistency:   {param_success}/{total_protocols} protocols")

        # Framework integration
        framework_success = sum(all_results['framework_integration'].values())
        print(f"Framework Integration:   {framework_success}/{total_protocols} protocols")

        # Metric consistency
        metric_success = sum(1 for p, metrics in all_results['metric_consistency'].items()
                           if all(metrics.values()))
        print(f"Metric Consistency:      {metric_success}/{total_protocols} protocols")

        overall_success_rate = (scenario_success + param_success + framework_success + metric_success) / (4 * total_protocols) * 100

        print(f"\nOverall Success Rate:    {overall_success_rate:.1f}%")

        if overall_success_rate >= 90:
            print("🎉 Excellent! Benchmarks are highly consistent across protocols.")
        elif overall_success_rate >= 75:
            print("✅ Good! Minor inconsistencies that should be addressed.")
        elif overall_success_rate >= 50:
            print("⚠️  Warning! Significant inconsistencies detected.")
        else:
            print("❌ Error! Major inconsistencies need immediate attention.")

        return all_results

    def generate_test_report(self, results: Dict[str, Any], output_file: str = "benchmark_consistency_report.txt"):
                ...
                if test_name == 'scenario_consistency':
                    for protocol, scenarios in test_results.items():
                        ...
                        if not missing and not extra:
                            f.write(f"    Status: ✓ All scenarios present\n")
                        f.write("\n")
