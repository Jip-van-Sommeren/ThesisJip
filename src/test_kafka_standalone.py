...
def test_kafka_standalone():
    ...
    try:
        ...
        assert len(received_messages) >= 1, "Should receive at least one message"
        ...
        agent2 = KafkaCommunicatingAgent("agent_2", kafka_service)
        ...
        print(f"Success rate: {stats['messages_delivered']/max(stats['messages_sent'], 1)*100:.1f}%")
