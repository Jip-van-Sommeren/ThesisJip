...
class PhysicsModelAgent(BatteryHybridAgent):
    ...
    def _initialize_reactive_rules(self):
        ...
        cycle_rule = ReactiveRule(
            condition=lambda state: "cycle_transition"
            in state.get("external", {}),
    ...
    def _agent_setup(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"PhysicsModelAgent setup complete for battery {self.battery_id}"
            )
            return True
    ...
    def _agent_teardown(self):
        ...
        logger.info(
            f"PhysicsModelAgent teardown complete for battery {self.battery_id}"
        )
        ...
        try:
            ...
            if msg.cycle != self.last_cycle_number:
                ...
                if self.current_cycle is not None:
                    ...
                    self.completed_cycles[completed_cycle.cycle] = (
                        completed_cycle
                    )

                    # Start new cycle first
                    self.current_cycle = CycleData(
                        cycle=msg.cycle, start_time=msg.timestamp
                    )
                    ...
                    logger.debug(
                        f"Completed cycle {completed_cycle.cycle}, started tracking cycle {msg.cycle}"
                    )
                else:
                    # First cycle
                    self.current_cycle = CycleData(
    ...
    def _handle_cycle_transition_for_cycle(self, completed_cycle: CycleData):
        ...
        try:
            ...
            pred_history = PredictionHistory(
                cycle=cycle, predicted_capacity=predicted_capacity
            )
        ...
        try:
            ...
            for pred in reversed(self.prediction_history):
                if pred.cycle == msg.cycle and pred.actual_capacity is None:
                    ...
                    self._publish_hybrid_training_sample(
    ...
    def _publish_hybrid_training_sample(
        self, cycle: int, actual_capacity: float
    ):
        ...
        if (
            ...
            and self.current_cycle.cycle == cycle
        ):
        ...
        duration = (
            cycle_data.total_charge_time
            or len(cycle_data.telemetry_points)
            or 1.0
        )
    ...
    def _adapt_parameters(self):
        ...
        try:
            ...
            if len(cycles) < 10:
                logger.warning(
                    "Not enough data for parameter adaptation (need >= 10 points)"
                )
                return
    ...
    def set_model_parameters(self, parameters: DegradationParameters):
        ...
        logger.info(
            f"Model parameters updated: k={parameters.k}, Q0={parameters.Q0}"
        )
