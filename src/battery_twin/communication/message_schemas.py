...
class TelemetryMessage(BaseModel):
    ...
    current: float = Field(..., description="Battery current in A (negative for discharge)")
    temperature: float = Field(..., description="Cell temperature in °C")
    ambient_temperature: Optional[float] = Field(None, description="Ambient temperature in °C")
    ...
    def validate_temperature(cls, v: float) -> float:
        if not (-20 <= v <= 80):
            raise ValueError(f"Temperature {v}°C outside valid range [-20, 80]°C")
        return v
...
class CapacityMessage(BaseModel):
    ...
    measurement_type: str = Field(default="measured", description="measured, estimated, or predicted")
...
class PredictionMessage(BaseModel):
    ...
    predicted_capacity: float = Field(..., ge=0, description="Predicted capacity in Ah")
    uncertainty: Optional[float] = Field(None, ge=0, description="Prediction uncertainty")
    horizon: int = Field(default=0, ge=0, description="Prediction horizon in seconds")
    ...
    def validate_prediction_type(cls, v: str) -> str:
        if v not in ['physics', 'ml', 'hybrid']:
            raise ValueError(f"Invalid prediction_type: {v}. Must be physics, ml, or hybrid")
        return v
...
class StateEstimateMessage(BaseModel):
    ...
    uncertainty: Optional[Dict[str, float]] = Field(
        None,
        description="Uncertainty estimates for soc, soh"
    )
    ...
    def validate_resistance(cls, v: Dict[str, float]) -> Dict[str, float]:
        ...
        if not all(key in v for key in required_keys):
            raise ValueError(f"internal_resistance must contain keys: {required_keys}")
        return v
...
class ParameterMessage(BaseModel):
    ...
    confidence: float = Field(..., ge=0, le=1, description="Confidence in parameters")
    fit_quality: Optional[Dict[str, float]] = Field(
        None,
        description="Fit quality metrics (RMSE, R2, etc.)"
    )
...
class FaultMessage(BaseModel):
    ...
    battery_id: str
    timestamp: float
    severity: str = Field(..., description="Severity level: warning, fault, critical")
    fault_type: str = Field(..., description="Type of fault detected")
    cause: str = Field(..., description="Human-readable cause description")
    residual_magnitude: float = Field(..., description="Magnitude of residual or anomaly")
    ...
    def validate_severity(cls, v: str) -> str:
        if v not in ['warning', 'fault', 'critical']:
            raise ValueError(f"Invalid severity: {v}. Must be warning, fault, or critical")
        return v
...
class ControlMessage(BaseModel):
    ...
    battery_id: str
    timestamp: float
    control_mode: str = Field(..., description="Control mode: CC, CV, CCCV, idle")
    current_setpoint: Optional[float] = Field(None, description="Current setpoint in A")
    voltage_limit: Optional[float] = Field(None, description="Voltage limit in V")
...
class AgentRegistrationMessage(BaseModel):
    ...
    """
    agent_id: str
    agent_type: str = Field(..., description="Agent type: BDI, Reactive, Hybrid")
...
class AgentHeartbeatMessage(BaseModel):
    ...
    agent_id: str
    timestamp: float
    status: str = Field(..., description="Status: active, idle, busy, degraded")
...
class AgentMetricsMessage(BaseModel):
    ...
    avg_processing_time: float = Field(default=0.0, ge=0, description="Average time in ms")
...
class CoordinationCommandMessage(BaseModel):
    ...
    command_type: str = Field(..., description="Command type: start, stop, restart, configure")
    ...
    def validate_command_type(cls, v: str) -> str:
        valid_commands = ['start', 'stop', 'restart', 'configure', 'optimize', 'reset']
        if v not in valid_commands:
            raise ValueError(f"Invalid command_type: {v}. Must be one of {valid_commands}")
        return v
...
class MessageFactory:
    ...
    @classmethod
    def create_message(cls, message_type: str, data: Dict[str, Any]) -> BaseModel:
        """
